...
class BadgeOrchestrator @Inject constructor(
    ...
    suspend fun getProssimoTurno(idDipendente: String): Resource<ProssimoTurno> {
        return try {
            ...
            val timbratureEsistenti = timbraturaDao.getByTurnoAndDipendente(turnoCorrente.id, idDipendente)
    ...
    fun createBadgeVirtuale(user: User, azienda: Azienda): BadgeVirtuale {
