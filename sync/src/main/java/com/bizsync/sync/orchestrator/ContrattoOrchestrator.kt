...
import com.bizsync.sync.sync.SyncContrattoManager
...
class ContrattoOrchestrator @Inject constructor(
    ...
    override suspend fun updateContratto(contratto: Contratto): Resource<String> {
        return try {
    ...
    override suspend fun getContratti(idAzienda: String, forceRefresh: Boolean): Resource<List<Contratto>> {
        return try {
            if (forceRefresh) {
